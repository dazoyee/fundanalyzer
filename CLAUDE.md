...
**フォームログイン認証**（Spring Security）。全リクエストを認証必須とし、単一のメモリ内ユーザーのみ。

- dev: `application.yml` の既定値（`admin` / `fundanalyzer-local-dev`）で起動可能
- prod: 環境変数 `SECURITY_USER` / `SECURITY_PASSWORD` が必須（未設定だと起動失敗）
