 ...
 * 企業価値算出に用いる業種別係数。
 *
 * <p>営業利益倍率・流動負債調整係数を保持する値オブジェクト。値は業種マスタ（industry 列）に由来し、
 * 既定値も列 DEFAULT（10 / 1.2）としてテーブル側で保持する。年換算重みは業種非依存の不変定数のため
 * 本オブジェクトは持たず {@code AnalysisResult.ANNUAL_WEIGHT} で定義する。
 */
@AllArgsConstructor
@Getter
