...
本ガイドラインの **唯一のタスク進行サイクル**。すべての開発タスクを **同じ 5 ステップ** で進める。

> **記号凡例**:
> - ★ … 必須・スキップ不可（Gate 1 / Gate 2）
> - 👤 … 人間レビュアの介入ポイント
> - 🤖 … AI エージェントが主導する作業（計画/実装エージェントの作業後に、検証エージェントが成果物を検証する）
...
flowchart TD
    ...
    subgraph S3 [ステップ 3: 設計（影響設計＋テスト設計）　👤 Gate 1]
        S3LEAD[計画エージェント主導]
        PRE{事前計画<br/>ドキュメント?}
    ...
    subgraph S4 [ステップ 4: 実行サイクル　🤖 AI 主導]
        DEV[実装エージェント — TDD]
        T[テストコード作成]
        I[実装]
        R{テスト実行}
...
| 対象外 | 実装しない（実装中に紛れ込みやすいが、本タスクのスコープ外とする事項） |
...
| **UI（画面）変更（該当する場合）** | 静的ページ／ノーコードツール／デザインツール |
| **複数の設計案を比較したいとき** | 各案を 1 つの HTML ファイルにグリッド配置し、レイアウト・トレードオフを横並びで比較（各案に「何を犠牲にしているか」を明記）|

> **複数案の横並び比較**: 方向性が定まらないタスク（UI の方向性・アルゴリズムの選択肢・API 設計の代替案など）では、AI に「はっきり異なる複数案」を生成させ、単一の HTML ファイルにグリッドで並べて比較すると認識ズレを早期に潰しやすい。この種の探索用 HTML は **使い捨ての提示層**（一次情報源ではない）として扱う（[human-checkpoints.md §一次情報源（Markdown）とレビュー提示層（HTML 等）の分離](human-checkpoints.md#一次情報源markdownとレビュー提示層html-等の分離) 参照）。合意した方向性は Markdown の計画・設計ドキュメントに確定させる。
...
## ステップ 3：設計（影響設計＋テスト設計）（★Gate 1）

**狙い**: 影響範囲とインフラ影響を機械的に洗い出して品質設計を確認・更新し、続けてテスト設計まで行って**完了条件を確定**する。影響設計とテスト設計を 1 つの設計フェーズとして束ね、Gate 1 で一括承認する。
...
| 状況 | 運用 |
|---|---|
| 事前計画ドキュメントで影響範囲・三本柱・スコープが **既に確定済** | タスク 1 Markdown ファイルの `## Gate 1` セクションは **事前計画への参照リンクのみ** で成立可（重複記載不要） |
...
実装前に「何のテストが必要か」を確定し、完了条件（機能 + テスト + ドキュメント ＋ スコープ外宣言）を固める。
...
- [ ] 完了条件が「機能 + テスト + ドキュメント」の 3 点セット ＋ スコープ外宣言で構成されている
...
- 完了条件（機能要件・テスト要件・ドキュメント要件・スコープ外宣言）が揃っているか

> 計画エージェントの設計作業（影響設計＋テスト設計）後に、検証エージェントが成果物を検証する（観点：影響範囲網羅性 / インフラ影響 / 三本柱整合性 / スコープ確定 / 依存追加判断 / 機能要件 / テスト要件 / ドキュメント要件 / スコープ外宣言 / 状態遷移マトリクス）。不合格時は計画エージェントに差し戻し、修正後に再検証。検証は Gate 1 の代替ではなく前段補助。詳細は [roles.md の検証エージェント章](roles.md#検証エージェントverifier) 参照。
...
> 実装エージェントは Gate 1 で承認された完了条件に基づいて作業に入る。詳細は [human-checkpoints.md](human-checkpoints.md)。
...
## ステップ 5：実装作業後の検証（★Gate 2）

**狙い**: 実装エージェントの作業後に、検証エージェントが 5 観点を **並列に** 検証し、観点の抜けを防ぐ。

> Gate 2 は人間が必ず最終確認する。詳細は [human-checkpoints.md](human-checkpoints.md)。

### 5 つの検証観点

5 観点の定義は [README.md §品質設計の三本柱と検証 5 観点の関係](README.md#品質設計の三本柱と検証-5-観点の関係) を一次情報源とする。
...
- [ ] Gate 1・Gate 2 がすべてタスク 1 Markdown ファイルに通過記録として残っている
- [ ] スコープ外の項目に手を出していない
...
| ドキュメントを最後にまとめて書く | ステップ 4 終盤 | タスク完了条件にドキュメント更新を含める |
| 影響範囲漏れ | ステップ 3 | 影響範囲分析を必ず実施・計画作業後の検証で網羅性を確認 |
| インフラ未考慮（タイムアウト等） | ステップ 3 | インフラ影響チェックリスト・計画作業後の検証で確認 |
| AI が完了条件にない機能を勝手に作る | ステップ 4 | 実装エージェントの禁止事項として明記 |
| テストが「動作確認の写し」になる | ステップ 4 | テスト戦略の品質特性に紐づくか検証エージェントが確認 |
| テストの歪曲（既存テストを書き換えて緑化） | ステップ 4 | 既存テスト変更禁止ルール |
| 完了条件の曖昧さで実装が暴走 | ステップ 3 | 計画作業後の検証で完了条件の充足性を確認 |
| 設計スコープ被り（複数のタスクが同じ箇所を同時改修して衝突する） | ステップ 3 | ドキュメント正規化（一次情報源） |
| ドキュメント陳腐化 | ステップ 5 | 静的解析・テストで自動検出できないため、検証エージェントが必ず目視確認 |
...
| 3 | 影響設計：参照層＝ステータス列挙型の全使用箇所を抽出／状態層＝状態遷移図を更新／データ層＝既存「下書き」レコードの移行戦略／インフラ＝バッチ実行時のロック戦略。テスト設計：状態遷移マトリクスで「承認待ち」の行・列を埋め、完了条件を確定（Gate 1 で一括承認） |
| 4 | テスト先行で実装。既存遷移テストは変更しない |
| 5 | 5 観点並列検証。特にドキュメント整合性を重点確認 |
...
| 2026-05-05 | 1.1.0 | 検証エージェントの実施タイミングを「計画/実装エージェントの作業後」と整理。ステップ 6 を「実装作業後の検証」にリネーム、各 Gate の補足注記から「多軸検証」用語を削除 |
| 2026-06-03 | 1.2.0 | ステップ 2 プロトタイピング手段に「複数の設計案を HTML グリッドで横並び比較」を追加。探索用 HTML を使い捨ての提示層と位置づけ、human-checkpoints.md の一次情報源／提示層分離へ相互参照 |
| 2026-06-16 | 2.0.0 | **6 ステップ → 5 ステップへ再構成**。旧ステップ 3「影響設計」と旧ステップ 4「テスト設計」を新ステップ 3「設計（影響設計＋テスト設計）」へ統合し Gate 1 で一括承認。旧ステップ 5/6 を新ステップ 4/5 へ繰り上げ。Gate を 2 ゲート（Gate 1＝設計の承認 / Gate 2＝最終確認）へ集約 |
