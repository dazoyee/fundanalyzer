...
- **(a) 一致検証**: 本番DBの対象行（document_type_code 120/130、bps 非NULL）全件に対し、
  ...
  調査再確認ルールに基づき同一クエリを2回実行し、同一結果を確認済み。
...
| 分類 | 件数 | 性質 |
|---|---|---|
| 5指標すべて NULL（= skipped 母集団） | 4,401 | 下記2要因に分解される |
| うち当期純利益の欠損 | 4,082 | PL はスクレイピング済み（scraped_pl='1'）だが「当期純利益」科目が抽出されていない。**再スクレイピングではなくキーワード照合の拡充が必要な領域** |
| うち企業マスタの証券コード NULL | 312 | 財務諸表の入力・業種係数はすべて揃っているが、`CompanySpecification.findCompanyByEdinetCode` が `code` 非保有の企業を除外するため skip。上場廃止等で EDINET 企業マスタ更新時に証券コードが失われた企業（全312件で code IS NULL を確認） |
| うち複合欠損（純資産・株式総数等） | 7 | 元データ欠損 |
...
- skipped 母集団全体では証券コード NULL の企業が 420 件あり、うち 108 件は当期純利益欠損とも重複する
- 集計クエリは調査再確認ルールに基づき2回実行し、同一結果を確認済み
- 312 件を救済する場合の論点: 上場廃止企業の指標をバックフィルする価値があるか（現行画面は上場中企業の
  バリュエーションが主目的）、および code 非保有企業を許容する場合の `findCompanyByEdinetCode` の
  フィルタ仕様変更の影響範囲。対応する場合は別タスクとして起票する

## 付随確認: 本番DBバックアップ表

データクレンジング時のバックアップ表（analysis_result / investment_indicator / corporate_view 系）は、
information_schema の全スキーマ検索（`%backup%` / `%negative%` パターン）で存在しないことを確認した。
fundanalyzer スキーマは通常テーブルのみで構成され、クリーンアップ対象は残っていない。
