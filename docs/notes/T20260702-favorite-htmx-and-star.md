...
| Gate 2: 最終確認   | 合格 | iori-oiso（人間レビュア） | 2026-07-02 |
...
- 動作確認結果（実機・実環境）: dev（H2、dev seed データ `db/dataset/V1.0.1`/`V1.0.2` の company 9001〜9010）で起動成功（Tomcat 8889）。画面確認は人間レビュア実施済み（2026-07-02）。Playwright ビジュアルベースラインを再取得し index/corporate の 4 ケースが緑であることを確認済み（詳細は AI 作業ログ §ステップ 5 追記分）。
- 副次影響: 旧 `POST /v2/favorite/company` 削除（参照 0 件確認）。既存 favorite 機能・テストに回帰なし（star 中核テスト 168 件緑）。副次的に発見した `ManualMobileScreenshotTest`（Playwright ベースライン撮影用テスト）の認証方式不具合（Basic 認証がログイン画面にリダイレクトされ撮影に失敗する既存バグ）を本 PR で修正済み。
...
| 回答日 | 2026-07-02 |
| **結論** | **合格** |
| **コメント** | 画面確認・Playwright ベースライン再取得（index/corporate 4 ケース緑）を確認して承認。なお `valuation`/`edinet-list`/`edinet-list-detail` の既存ベースラインはバージョン表記の経年変化・動的タイムスタンプにより恒常的に閾値超過する構造的問題を発見したが、本タスクのスコープ外（未変更画面）のため別タスク化する。 |
...
- **Gate 2: Playwright ビジュアルベースライン再取得**（追記）:
  - dev seed データ（`db/dataset/V1.0.1__dev_seed_screen_test.sql`/`V1.0.2__dev_seed_main_tie_break.sql`、company 9001〜9010 の架空データ）で dev 起動し撮影を試行したところ、`ManualMobileScreenshotTest`（撮影用テスト）が Basic 認証で接続しており、`SecurityConfig`（formLogin と httpBasic を両方設定）の仕様上ブラウザからの未認証アクセスは常に `/login` へのリダイレクトが優先されるため、**常にログイン画面を撮影してしまう既存不具合**を発見（今回の favorite/star 変更とは無関係）。
  - 判定側 `MobileScreenshotRegressionTest` が実装済みのフォームログイン方式（`/login` に POST しストレージステートを保持）と同じパターンへ `ManualMobileScreenshotTest` を修正（Codex 委譲、プロダクションコード非変更）。
  - 修正後、`index-desktop`/`index-mobile`/`corporate-desktop`/`corporate-mobile` の 4 ケースを再撮影・`src/test/resources/playwright-baselines/` を更新。`MobileScreenshotRegressionTest` で該当 4 ケースが緑であることを確認（許容閾値 2% 以内）。
  - 併せて `valuation`/`edinet-list`/`edinet-list-detail`（今回未変更の 3 画面・6 ケース）が既存ベースラインに対し 9.9%〜16.4% の差分で恒常的に失敗することを発見。差分画像を確認した結果、原因は (a) アプリバージョン表記の経年変化（フッター `v2.3.2` → `v2.3.11-SNAPSHOT`）、(b) `edinet-list` 画面の「会社リスト更新日時」がリアルタイム現在時刻を表示する動的コンテンツであること、の 2 点。テンプレート・Presenter は今回一切変更しておらず、本タスクのスコープ外の既存構造的問題と判断（別タスク化を推奨）。
...
## 添付ファイル参照
- [T20260702-attachments/gate2-index-desktop.png](T20260702-attachments/gate2-index-desktop.png) — 会社一覧（desktop、お気に入り／星ボタン・絞り込みタブ確認用）
- [T20260702-attachments/gate2-index-mobile.png](T20260702-attachments/gate2-index-mobile.png) — 会社一覧（mobile）
- [T20260702-attachments/gate2-corporate-desktop.png](T20260702-attachments/gate2-corporate-desktop.png) — 銘柄詳細（desktop、お気に入り htmx トグル・星ボタン確認用）
- [T20260702-attachments/gate2-corporate-mobile.png](T20260702-attachments/gate2-corporate-mobile.png) — 銘柄詳細（mobile）

## 更新履歴
| 日付 | 概要 |
|---|---|
| 2026-07-02 | 起票。認識合わせ→設計（Gate 1 合格）→実装（Codex 委譲）→検証（code-reviewer・テスト 168 件緑）→PR #283 作成→Playwright ベースライン再取得（副次的に `ManualMobileScreenshotTest` の認証方式不具合を発見・修正）→Gate 2 合格を記録。残課題: `valuation`/`edinet-list`/`edinet-list-detail` の既存ベースライン陳腐化（別タスク化推奨、本タスクのスコープ外）。 |
